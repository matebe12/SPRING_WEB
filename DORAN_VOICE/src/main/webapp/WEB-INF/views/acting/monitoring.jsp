...
<body>
	...
	<div class="container">
		...
		<div id="doran">
			<h1
				style="font-family: 'Noto Sans KR', sans-serif; font-size: 35pt; font-weight: bold; padding-bottom: 10px;">
				연기 모니터링
			</h1>
		</div>
		...
		<c:if test="${pageMaker.prev}">
			...
			<a
				href='<c:url value="/monitoring?page=${pageMaker.startPage - 1}"/>'>«</a>
		...
		<c:forEach begin="${pageMaker.startPage }" end="${pageMaker.endPage }"
			...
			<a href='<c:url value="/monitoring?page=${idx}"/>'
		...
		<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
			...
			<a href='<c:url value="/monitoring?page=${pageMaker.endPage + 1}"/>'>
				» </a>
